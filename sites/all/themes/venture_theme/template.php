...
function venture_theme_preprocess_page(&$vars) {
	...
	$node=node_load(9);
	//print_r($vars["page"]["content"]);
	$slide=array();	
	if(isset($node->field_slide)) {
		
		foreach($node->field_slide["und"] as $data) {
			$slide[]=array(
			"titre"=>$data["field_titre"]["und"][0]["value"],
			"sous_titre"=>$data["field_sous_titre"]["und"][0]["value"],
			"image"=>$data["field_image"]["und"][0]["filename"]
			);
		}
		$vars["slider"]=$slide;
...
function venture_theme_preprocess_node(&$variables) {
 ...
 // print_r($node);
  if($node->type=="formation") {
  		$id=$variables["node"]->field_id_pcp["und"][0]["value"];
  		$variables["node"]->field_id_pcp=null;
  		$variables["inscription"]=$id;
